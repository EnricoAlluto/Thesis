...
public class AvatarFinishManager : MonoBehaviour
{
    ...
    private const int MAX_SAVED_AVATARS = 1;
    ...
    private IEnumerator FinishAvatar()
    {
       ...
       // Debug.Log("[FinishAvatar] Inizio salvataggio GLB...");
        ...
        {
            //Debug.Log("[FinishAvatar] Tutti i container sono pieni, sostituirò l'avatar più vecchio");
            containerLibero = 0;
            
            if (containerAvatars[containerLibero] != null && containerAvatars[containerLibero].transform.childCount > 0)
            {
                AvatarSaveData oldestAvatar = null;
                foreach (var avatar in loadedAvatars)
                {
                    if (oldestAvatar == null || 
                        (DateTime.TryParse(avatar.creationDate, out DateTime currentDate) && 
                         DateTime.TryParse(oldestAvatar.creationDate, out DateTime oldestDate) && 
                ...
                if (oldestAvatar != null && !string.IsNullOrEmpty(oldestAvatar.saveDirectory))
                {
                    try
                    {
                        //Debug.Log($"[FinishAvatar] Eliminazione dei file dell'avatar più vecchio in: {oldestAvatar.saveDirectory}");
                        Directory.Delete(oldestAvatar.saveDirectory, true);
                        loadedAvatars.Remove(oldestAvatar);
                    }
                    catch (Exception ex)
                    {
                        //Debug.LogError($"[FinishAvatar] Errore durante l'eliminazione dei file: {ex.Message}");
                    }
                }
                ClearContainer(containerAvatars[containerLibero]);
            }
        }
    ...
    public string GetSaveDirectory()
    {
        ...
        {
            //.LogError($"Errore nell'accesso alla directory: {e.Message}");
            return null;
        }
    }
    ...
    private IEnumerator SalvaAvatarComeGLBCoroutine(System.Action<bool, string> onComplete)
    {
        ...
        try
        {
            var allAvatarDirs = Directory.GetDirectories(saveDir)
                .Where(d => Directory.GetFiles(d, "*.glb").Any())
                .OrderBy(d => Directory.GetCreationTime(d))
                .ToList();

            if (allAvatarDirs.Count >= MAX_SAVED_AVATARS)
            {
                var oldestDir = allAvatarDirs.First();
                //Debug.Log($"[SalvaAvatarComeGLBCoroutine] Raggiunto il numero massimo di avatar. Elimino il più vecchio: {oldestDir}");
                Directory.Delete(oldestDir, true);
                //Debug.Log($"[SalvaAvatarComeGLBCoroutine] Vecchio avatar eliminato con successo");
            }
        }
        catch (Exception ex)
        {
            //Debug.LogError($"[SalvaAvatarComeGLBCoroutine] Errore durante la gestione dei vecchi avatar: {ex.Message}");
        }
