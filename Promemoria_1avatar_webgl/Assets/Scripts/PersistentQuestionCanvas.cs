...
public class PersistentQuestionCanvas : MonoBehaviour
{
    ...
    public void ShowCanvas(bool show)
    {
        if (questionCanvas != null)
        {
            ...
            if (show)
            {
                ...
                if (canvas2 != null && currentRoomIndex >= 3) canvas2.gameObject.SetActive(false);
    ...
    private IEnumerator SendAvatarAccessData(string question, string response, string score, string dateTime)
    {
        ...
        string jsonData = JsonUtility.ToJson(new Serialization<string, string>(dataObject));
        
        string postData = $"player_name={UnityWebRequest.EscapeURL(playerName)}&" +
                         $"id_domanda={UnityWebRequest.EscapeURL(question)}&" +
                         $"risposta={UnityWebRequest.EscapeURL(response)}&" +
                         $"score={UnityWebRequest.EscapeURL(score)}&" +
                         $"date_and_time={UnityWebRequest.EscapeURL(dateTime)}";
    ...
    private class Serialization<TKey, TValue>
    {
        ...
        public Serialization(Dictionary<TKey, TValue> dictionary)
        {
            ...
            {
                list.Add(new KeyValuePair<TKey, TValue>(kvp.Key, kvp.Value));
            }
        }
    }
